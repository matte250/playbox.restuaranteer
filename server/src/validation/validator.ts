export const validate = (
			...
			failed: [
				{
					type: 'required',
					keyTrace: key,
					msg: $lbl ? `${$lbl} cant be empty` : `${key} is undefined`,
				} as FailedValidation,
			],
			onlyMsg,
		};

	const defKeys = Object.keys(validationDef).filter(
		(x) => !x.startsWith('$'),
	);
	...
	defKeys.forEach((x) => {
		...
		if (res) failed = failed.concat(res);
	});
	...
	if ($map) {
		if (!Array.isArray(value)) {
			...
			const func = $brk ? value.forEach : value.some;
			func((x) => {
...
const onlyMessages = (failed: ValidationResults) =>
	failed.reduce((pre, cur) => pre.concat(cur.msg), [] as string[]);

export const createValidatorFunctions = (funcs: ValidationFunctions) =>
